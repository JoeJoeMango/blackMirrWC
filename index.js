...
import shader from '../shader/blackMirrShader.glsl';
import wrapper from '/blackMirrShader.ts';
...
async function loadVideo(cameraLabel) {
  ...
  state.video.play();
  // state.video.stopExistingVideoCapture();
}
...
function setupGui(cameras) {
  ...
  console.log(guiState.segmentation);
...
//--------------//--------------//--------------//--------------//--------------//--------------
            ...
            const mask = bodyPix.toMaskImageData(
                ...
                var gray = 255;
                //mask is a Uint8ClampedArray
                console.log(mask.data);
            for(var i = 0; i < mask.data.length; i+=4){

            var r = mask.data[i] = gray;
            var g = mask.data[i+1] = 20;
            var b = mask.data[i+2] = gray;
            var a = mask.data[i+3];
          ...
          // this is where I am applying the shader
                //this applies the shader to the whole video
                renderer.addImage(state.video,0);
                // this applies it to the mask
                 // renderer.addImage(mask,0);
            break;
          case 'bokeh':
...
export async function bindPage() {
  ...
  console.log( segmentBodyInRealTime());
}
